...
class DiscreteActorCritic(DiscreteRL):
    ...
    #         logits = out[0]
    #     elif len(out)==5:
    #         distr, qvalues = out[0], out[1]
    #         representation, repr_var = out[3], out[4]
    #     else:
    #         pass 
...
class ContinuousActorCritic(ContinousRL):
    ...
    #     self.train_on_tensors(states, actions)

    # def train_on_tensors(self, states: torch.Tensor, actions: torch.Tensor, rewards: torch.Tensor, cumulative_returns: torch.Tensor, expl: float = 1.0, repre: float = 1.0):
